...
async def main():
    ...
    # 步骤2：选择分析模式
    with st.expander("第二步：选择分析模式", expanded=True):
    ...
    # 步骤3：执行设置（并行度、分数阈值等）
    with st.expander("第三步：高级设置(可选)", expanded=False):
            ...
            try:
                ...
                for video_path in video_paths:
                    ...
                    if srt_files:
                        ...
                        st.info(f"已读取视频 {video_base_name} 的现有字幕文件，共 {len(subtitle_df)} 条记录")
                    else:
                        # 提取新字幕
                        st.info(f"正在提取视频 {video_base_name} 的字幕...")
